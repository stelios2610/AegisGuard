#!/bin/bash
# AegisGuard Offline ISO Builder (WSL2)
...
OUTPUT='/mnt/c/Users/stelakis-pc/Projects/firewall-gui/build/AegisGuard-1.0.0-offline-amd64.iso'
WORK='/tmp/aegisguard-iso-offline'
...
echo ""
echo -e "${C}  ╔══════════════════════════════════════════════╗${NC}"
echo -e "${C}  ║   AegisGuard OFFLINE ISO Builder             ║${NC}"
...
menuentry "Install AegisGuard Network Security (Offline)" --class ubuntu --class os {
    set gfxpayload=keep
...
info "[5/8] Embedding AegisGuard source code (offline — no GitHub)..."
mkdir -p "$CUSTOM/aegisguard_app"
...
rsync -a --delete \
    ...
    "$SRC_WIN/" "$CUSTOM/aegisguard_app/"

log "App code embedded: $(find "$CUSTOM/aegisguard_app" -name '*.py' | wc -l) Python files"
...
echo "=== AegisGuard Offline Install: $(date) ==="
...
/opt/aegisguard/venv/bin/pip install --quiet --upgrade pip \
    --no-index --find-links /cdrom/aegisguard_wheels/ 2>/dev/null || \
/opt/aegisguard/venv/bin/pip install --quiet --upgrade pip

/opt/aegisguard/venv/bin/pip install --quiet \
    --no-index --find-links /cdrom/aegisguard_wheels/ \
...
# ── systemd services ──────────────────────────────────────────────────────────
cp /cdrom/server-configs/aegisguard.service          /etc/systemd/system/aegisguard.service
cp /cdrom/server-configs/aegisguard-firstboot.service /etc/systemd/system/aegisguard-firstboot.service

# ── first-boot.sh ─────────────────────────────────────────────────────────────
cp /cdrom/server-configs/first-boot.sh /opt/aegisguard/build/first-boot.sh
...
echo ""
echo -e "${G}  ╔══════════════════════════════════════════════╗${NC}"
echo -e "${G}  ║   AegisGuard OFFLINE ISO built!              ║${NC}"
echo -e "${G}  ║                                              ║${NC}"
echo -e "${G}  ║   File: build/AegisGuard-1.0.0-offline...   ║${NC}"
